...
# SSH
read -p "Enter the ssh port number to delete the rule (default 22): " SSH_PORT
...
# X-UI web panel
read -p "Enter the X-UI web panel port number to delete the rule (empty if none): " XUI_PORT
if [ -n "$XUI_PORT" ]; then
...
echo "iptables rules were successfully removed"
